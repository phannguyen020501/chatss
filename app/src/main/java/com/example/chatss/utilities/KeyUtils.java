...
public class KeyUtils {
    ...
    private static final File file = new File(Environment.getExternalStorageDirectory(), KEYSTORE_FILENAME);
...
//        System.out.println("public key a: " + Hex.toHexString(keyPairA.getPublic().getEncoded()));
        ...
        SecretKey secretKeyA = generateSharedSecret(privateKey1, publicKey2);
        SecretKey secretKeyB = generateSharedSecret(privateKey2, publicKey1);
    ...
    public static void savePrivateKey(String email, String password, KeyPair keyPair) throws CertificateException, KeyStoreException, IOException, NoSuchAlgorithmException, SignatureException, NoSuchProviderException, InvalidKeyException {
...
//        KeyPair keyPair =generateECKeys();
        ...
        KeyStore keyStore;
        if (new File(Environment.getExternalStorageState(),KEYSTORE_FILENAME).exists()){
            keyStore = loadKeyStore();
        }else {
    ...
    public static PrivateKey getPrivateKeyFromKeyStore(String alias, String keyPassword) {
        try {
            // Load the KeyStore
            KeyStore keyStore =loadKeyStore();
...
//        Security.addProvider(new BouncyCastleProvider());
    ...
    public static KeyStore loadKeyStore() throws KeyStoreException, IOException, CertificateException, NoSuchAlgorithmException, SignatureException, NoSuchProviderException, InvalidKeyException {
